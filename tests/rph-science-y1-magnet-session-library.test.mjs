...
assert.equal(select({schedule:{...schedule,end_time:'15:00:00'}}).status,'DURATION_MISMATCH');
assert.equal(select({lessonDate:'2026-09-08'}).status,'SELECTED');
assert.equal(select({lessonDate:'2026-09-10',schedule:null}).status,'SELECTED');
assert.equal(select({lessonDate:'not-a-date'}).status,'CALENDAR_NOT_CONFIRMED');
...
assert.equal(ped.activityLibrarySelection.lessonDateMode,'flexible');
for(const route of api.ROUTES)assert.equal(api.rowReady(readyRow(route),route),true,`route M${route.week}/S${route.session}`);

console.log('RPH Science Y1 Magnet flexible date: PASS');
