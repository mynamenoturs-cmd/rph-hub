...
function rptWeekDisposition(text='',weekNo=null){
  const clean=normalizeText(text);if(!clean)return {blocked:false,reason:'',block:''};
  const exact=weekNo?extractWeekContext(clean,weekNo):'';
  const block=(exact||clean).slice(0,5200);
  const rules=[
    {re:/\bNO\s*[-–—:]\s*(?:UASA|ujian|pentaksiran|pengurusan|cuti|orientasi|transisi)\b/i,reason:'RPT menandakan minggu ini sebagai NO RPH biasa'},
    {re:/\b(?:UASA|Ujian Akhir Sesi Akademik|Pentaksiran Akhir)\b/i,reason:'minggu pentaksiran akhir/UASA'},
    {re:/\bPengurusan Akhir Tahun\b/i,reason:'minggu pengurusan akhir tahun'},
    {re:/\b(?:cuti penuh|cuti penggal|cuti akhir persekolahan|school holiday|term break)\b/i,reason:'minggu cuti'},
    {re:/\b(?:Program Transisi|Minggu Orientasi|Orientation Week)\b/i,reason:'minggu orientasi/transisi'}
  ];
  const hit=rules.find(x=>x.re.test(block));
  return {blocked:Boolean(hit),reason:hit?.reason||'',block};
}
function lessonMapWeekDisposition(map){
  const meta=map?.source_evidence?.meta||map?.evidence?.meta||{};
  if(meta.non_teaching_week)return {blocked:true,reason:meta.non_teaching_reason||'minggu bukan PdP biasa',block:''};
  const ev=map?.source_evidence||map?.evidence||{};
  return rptWeekDisposition([map?.title,ev?.rpt?.text,ev?.rpt?.ref].filter(Boolean).join('\n'),map?.week_no);
}
...
function weekCoverageFromRptChunks(rptChunks=[],f=currentMapFilter()){
  ...
  if(!best){
    ...
    const rawWeekText=weekTexts.join('\n\n').slice(0,4800),disposition=rptWeekDisposition(rawWeekText,f.week_no);
    return {mode:'legacy-rpt',enforce:false,week:f.week_no,sessions:[],expected:0,completeCount:0,uniqueActivities:0,sourceComplete:!disposition.blocked,doc:null,score:0,rawWeekText,nonTeaching:disposition.blocked,nonTeachingReason:disposition.reason};
  }
  const disposition=rptWeekDisposition(best.sessions.map(x=>x.context).join('\n'),f.week_no);best.nonTeaching=disposition.blocked;best.nonTeachingReason=disposition.reason;if(disposition.blocked)best.sourceComplete=false;
...
function renderWeekCoverage(cov=currentWeekCoverage,f=currentMapFilter()){
  ...
  const en=lessonLanguage(f.subject_id)==='en';if(cov?.nonTeaching){const rawText=cov?.rawWeekText||'';box.innerHTML=`<div class="week-coverage-head"><div><div class="eyebrow">${en?'NON-TEACHING WEEK':'MINGGU BUKAN PDP BIASA'}</div><h3>${en?'No regular lesson plan required':'RPH biasa tidak perlu dijana'}</h3></div><span class="pill demo">NO RPH</span></div><div class="week-coverage-warning">⚠ ${escapeHtml(cov.nonTeachingReason||'RPT menandakan minggu ini sebagai pentaksiran, cuti atau pengurusan akhir tahun.')}</div>${rawText?`<details class="rpt-week-preview"><summary>${en?'View RPT content for Week '+f.week_no:'Lihat kandungan RPT Minggu '+f.week_no}</summary><pre class="rpt-interactive">${renderInteractiveRptText(rawText,f.week_no)}</pre></details>`:''}`;return}if(!cov||!cov.enforce){const rawText=cov?.rawWeekText||'';box.innerHTML=`<div class="week-coverage-head"><div><div class="eyebrow">${en?'WEEKLY SOURCE COVERAGE':'LIPUTAN SUMBER MINGGUAN'}</div><h3>${en?'Stable session IDs not detected':'ID sesi stabil belum dikesan'}</h3></div><span class="pill demo">INFO</span></div><p class="field-note">${en?'This Scheme of Work does not expose stable per-session IDs, so weekly completeness is not enforced.':'RPT ini tidak mempunyai ID seperti BM2-M29-S1. Sistem tidak akan meneka jumlah sesi; gunakan RPT murni untuk semakan liputan penuh.'}</p>${rawText?`<details class="rpt-week-preview"><summary>${en?'View RPT content for Week '+f.week_no:'Lihat kandungan RPT Minggu '+f.week_no}</summary><pre class="rpt-interactive">${renderInteractiveRptText(rawText,f.week_no)}</pre><p class="field-note">${en?'Click SP codes, titles or BT references to auto-fill Lesson Map fields.':'Klik kod SP, tajuk atau rujukan BT untuk auto isi medan Lesson Map.'}</p></details>`:''}`;return}
...
async function buildLessonCandidate(){if(!requireAuth())return;
  ...
  const weeklyCoverage=weekCoverageFromRptChunks(rptChunks,f);currentWeekCoverage=weeklyCoverage;renderWeekCoverage(weeklyCoverage,f);if(weeklyCoverage.nonTeaching){state.lessonCandidate=null;$('#mapEvidence').innerHTML=`<div class="evidence-card miss"><div><b>NO RPH BIASA • Minggu ${f.week_no}</b></div><p>${escapeHtml(weeklyCoverage.nonTeachingReason||'RPT menandakan minggu ini sebagai bukan minggu PdP biasa.')}. Sistem tidak akan membina atau mengesahkan Lesson Map biasa untuk minggu ini.</p></div>`;renderMapGate(null,false);return toast(`Minggu ${f.week_no}: ${weeklyCoverage.nonTeachingReason||'bukan minggu PdP biasa'}. RPH biasa tidak dijana.`,6500)}const exactRptSession=weeklyCoverage.sessions?.find(x=>Number(x.session)===Number(f.session_no))||null;if(weeklyCoverage.enforce&&!exactRptSession){state.lessonCandidate=null;renderMapGate(null,false);return toast(`Sesi ${f.session_no} tidak ditemui dalam RPT murni Minggu ${f.week_no}. Sistem tidak akan jatuh balik ke sesi/jadual lain.`,6000)};
...
function renderRphLessonOptions(){
  ...
  el.innerHTML=
    ...
    maps.map(x=>{
      ...
      const disposition=lessonMapWeekDisposition(x);
      ...
      const status=disposition.blocked?`NO RPH • ${disposition.reason}`:(verified?'✓ Disahkan':'○ Belum disahkan');
      return `<option value="${x.id}" ${verified&&matchesSchedule&&!disposition.blocked?'':'disabled'}>
  ...
  if(route.available&&route.session_no){
    const exact=maps.find(x=>Number(x.session_no)===Number(route.session_no)&&x.verification_status==='verified'&&!lessonMapWeekDisposition(x).blocked);
    el.value=exact?.id||'';
  }
}
...
async function listAccessibleClassrooms(token){
  const courses=[];let pageToken='';
...
function ensureRphClassroomBrowser(){
  let panel=$('#rphClassroomBrowser');
  if(panel)return panel;

  const gate=$('#rphAccuracyGate');
  if(!gate)return null;

  const email=escapeHtml(state.user?.email||'');

  panel=document.createElement('section');
  panel.id='rphClassroomBrowser';
  panel.className='card classroom-picker rph-classroom-browser';

  panel.innerHTML=`
    <div class="classroom-picker-head">
      <div>
        <b>Google Classroom</b>
        <small>
          Buka Classroom dan Assignment tanpa perlu Generate RPH.
        </small>
      </div>

      <button id="loadClassrooms"
              class="ghost"
              type="button">
        🔗 Connect Classroom
      </button>
    ...
    <div class="drive-account-picker">
      <label>
        Google Workspace account
        <select id="rphDriveAccountMode">
          <option value="current">
            Use signed-in DELIMa account${email?': '+email:''}
          </option>
          <option value="other">
    ...
    <div class="classroom-picker-grid">
      <label>
        Classroom
        <select id="rphClassroomCourse">
          <option value="">Connect Classroom dahulu</option>
        </select>
      </label>

      <label>
        Topic (Teacher)
        <select id="rphClassroomTopic">
          <option value="">Tiada topik dipilih</option>
        </select>
      </label>
    </div>

    <label class="classroom-auto">
      <input id="rphClassroomAuto" type="checkbox">
      <span>Auto cipta draf selepas Simpan RPH</span>
    </label>

    <div id="rphStudentAddWork"
         class="classroom-picker-grid"
         hidden>
    ...
    <small id="rphClassroomStatus"
           class="classroom-status">
      Connect Classroom untuk melihat kelas dan Assignment.
    </small>
  `;

  gate.insertAdjacentElement('afterend',panel);

  $('#loadClassrooms')?.addEventListener(
    'click',
    connectGoogleClassroom
  );

  $('#rphClassroomCourse')?.addEventListener(
    'change',
    refreshClassroomTarget
  );

  $('#rphDriveAccountMode')?.addEventListener(
    'change',
    resetDriveToken
  );

  $('#rphClassroomAssignment')?.addEventListener(
    'change',
    updateClassroomAssignmentActions
  );
...
function updateClassroomAssignmentActions(){
  const courseId=$('#rphClassroomCourse')?.value||'';
  const course=CLASSROOM_COURSES.find(x=>x.id===courseId);

  const assignmentId=
    $('#rphClassroomAssignment')?.value||'';

  const assignment=
    CLASSROOM_ASSIGNMENTS.find(x=>x.id===assignmentId);

  const panel=$('#rphStudentAddWork');
  const add=$('#addRphMyWork');
...
async function refreshClassroomAssignments(){
  ensureRphClassroomBrowser();
  ...
  try{
    setClassroomStatus(
      course?.canPublish
        ? 'Mencari Assignment untuk akaun Teacher…'
        : 'Mencari Assignment untuk akaun Student…'
    );

    const token=
      await requestDriveToken(driveAccountMode());
    ...
    setClassroomStatus(
      CLASSROOM_ASSIGNMENTS.length
        ? `${CLASSROOM_ASSIGNMENTS.length} Assignment ditemui • ${course?.canPublish?'Teacher':'Student'}.`
        : 'Tiada Assignment ditemui.',
      'ok'
    );
...
async function refreshClassroomTarget(){
  ...
  if(course?.canPublish){
    ...
    if(topic)
      topic.innerHTML=
        '<option value="">Tidak digunakan untuk Student</option>';
  }

  await refreshClassroomAssignments();
}
...
async function sendGeneratedRphToClassroom({auto=false}={}){
  const ctx=state.currentGeneratedRph;if(!ctx)return toast('Generate RPH dahulu.');const courseId=$('#rphClassroomCourse')?.value,topicId=$('#rphClassroomTopic')?.value||'',course=CLASSROOM_COURSES.find(x=>x.id===courseId);if(!courseId){toast(auto?'Auto hantar belum dijalankan. Hubungkan dan pilih Google Classroom dahulu.':'Hubungkan dan pilih Google Classroom dahulu.',6000);return false}if(!course?.canPublish){toast('RPH hanya boleh dihantar ke Classroom yang anda ajar sebagai guru.',6000);return false}if(ctx.classroomDraftCourses?.includes(courseId)){toast('RPH ini sudah dihantar sebagai draf ke Classroom yang dipilih.',5000);return true}
  const btn=$('#sendRphClassroom');if(btn)btn.disabled=true;setClassroomStatus('Upload RPH dan cipta Material draf…');
  try{const token=await requestDriveToken(driveAccountMode()),{file}=await ensureGeneratedRphDriveFile(ctx,token),material=await createClassroomDraft(token,courseId,ctx,file,topicId);ctx.classroomDraftCourses=[...(ctx.classroomDraftCourses||[]),courseId];setClassroomStatus(`Draf berjaya dihantar ke ${course?.name||'Google Classroom'}.`,'ok');toast(`Draf Google Classroom berjaya: ${course?.name||material.title}`,7000);return true}catch(error){setClassroomStatus(`Hantar gagal: ${error.message}`,'bad');toast('Google Classroom gagal: '+error.message,8000);return false}finally{if(btn)btn.disabled=false}
}
