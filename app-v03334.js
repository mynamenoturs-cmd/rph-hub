...
const demo = {
  classes:[{id:'c1',name:'1 Crystal',year:1},{id:'c2',name:'1 Topaz',year:1},{id:'c3',name:'3 Sapphire',year:3}],
  subjects:[{id:'s1',code:'BM',name:'Bahasa Melayu'},{id:'s2',code:'PJ',name:'Pendidikan Jasmani'},{id:'s3',code:'PK',name:'Pendidikan Kesihatan'}],
  standards:[
    {id:'sp1',subject_id:'s1',year:1,code:'2.1.1',description:'Membaca dan memahami bahan.'},
    {id:'sp2',subject_id:'s2',year:1,code:'1.8.4',description:'Melakukan guling depan dengan lakuan yang betul dan selamat.'},
    {id:'sp3',subject_id:'s3',year:3,code:'7.1.3',description:'Mengaplikasikan langkah mengelakkan situasi yang mengancam keselamatan diri.'}
  ],
  students:['Aiman Hakim','Aina Sofea','Amirul Haziq','Aqilah Humaira','Arif Danish','Balqis Imani','Daniel Lee','Dayang Nur','Farhan Akmal','Hannah Lim','Irfan Zikri','Khairol Aiman','Maisarah','Muhammad Adam','Nur Alya','Qistina','Rayyan','Siti Aisyah','Syafiq','Zara Aleesya'].map((name,i)=>({id:'m'+(i+1),class_id:'c1',name,student_no:i+1})),
  rpt:[{id:'r1',subject_id:'s1',year:1,week_no:29,title:'Contoh Pelajaran Minggu 29',sk:'2.1 Asas membaca',sp:'2.1.1 Membaca dan memahami bahan.',objective:'Pada akhir PdP, murid dapat membaca dan memahami bahan dengan bimbingan yang sesuai.',success_criteria:'Murid dapat memberi respons berdasarkan bahan yang dipelajari.',suggested_activities:'Aktiviti ini hanyalah data DEMO. Untuk penggunaan sebenar, upload RPT/DSKP/buku atau import rekod RPT sebenar.',textbook_ref:'Sumber demo',source_ref:'Demo'}]
};
...
async function getRptWeekRangesForRph(subjectId,year,academicYear){
  ...
  if(state.connected&&state.user){const {data,error}=await state.client.from('source_chunks').select('document_id,chunk_no,content').in('document_id',ids).order('chunk_no').limit(5000);if(error){console.warn('RPT week resolver',error);return[]}rows=data||[]}
  else rows=state.sourceChunks.filter(c=>ids.includes(c.document_id));
...
async function loadAll(){if(!requireAuth())return;
  ...
  const qs=await Promise.all([
    c.from('classes').select('*').order('year').order('name'),
    c.from('subjects').select('*').order('name'),
    c.from('curriculum_standards').select('*').order('year').order('code'),
    c.from('students').select('*').eq('is_active',true).order('name'),
    c.from('transit_records').select('*').order('assessment_date',{ascending:false}).limit(2000),
    c.from('book_checks').select('*').order('check_date',{ascending:false}).limit(2000),
    c.from('rpt_lessons').select('*').order('week_no').limit(2000),
    c.from('source_documents').select('*').order('created_at',{ascending:false}).limit(1000),
    c.from('timetable_entries').select('*').order('day_of_week').order('start_time').limit(1000),
    c.from('lesson_maps').select('*').order('academic_year',{ascending:false}).order('week_no').order('session_no').limit(3000),
    c.from('rph_records').select('*').order('lesson_date',{ascending:false}).limit(2000),
    c.from('rph_activity_history').select('*').order('lesson_date',{ascending:false}).limit(4000)
  ]);
...
function subscribeRealtime(){
  ...
  state.client.channel('hub-live')
    .on('postgres_changes',{event:'*',schema:'public',table:'transit_records'},()=>state.access?.status==='allowed'&&loadAll())
    .on('postgres_changes',{event:'*',schema:'public',table:'book_checks'},()=>state.access?.status==='allowed'&&loadAll())
    .on('postgres_changes',{event:'*',schema:'public',table:'source_documents'},()=>state.access?.status==='allowed'&&loadAll())
    .on('postgres_changes',{event:'*',schema:'public',table:'lesson_maps'},()=>state.access?.status==='allowed'&&loadAll()).subscribe();
  const email=normEmail(state.user.email);
...
function refreshStandards(){const subject=$('#transitSubject').value,cls=getClass($('#transitClass').value);if(!cls)return;const arr=state.standards.filter(x=>x.subject_id===subject&&Number(x.year)===Number(cls.year));$('#transitStandard').innerHTML=arr.length?arr.map(x=>`<option value="${x.id}">${escapeHtml(x.code)} — ${escapeHtml(x.description)}</option>`).join(''):'<option value="">Belum ada SP — upload DSKP/RPT dan Auto Detect</option>'}
function classStudents(classId){return state.students.filter(x=>x.class_id===classId)}
function renderTransitRows(){const list=classStudents($('#transitClass').value);$('#transitRows').innerHTML=list.map((s,i)=>`<tr data-student="${s.id}"><td>${i+1}</td><td><b>${escapeHtml(s.name)}</b></td><td><select class="tp"><option value="">—</option>${[1,2,3,4,5,6].map(n=>`<option value="${n}">TP${n}</option>`).join('')}</select></td><td><input class="note" placeholder="Evidens / pemerhatian ringkas"></td></tr>`).join('')||'<tr><td colspan="4">Tiada murid untuk kelas ini. Import murid di HUD Setup Data.</td></tr>'}
...
function renderBookRows(){const list=classStudents($('#bookClass').value);$('#bookRows').innerHTML=list.map((s,i)=>`<tr data-student="${s.id}"><td>${i+1}</td><td><b>${escapeHtml(s.name)}</b></td><td><select class="status"><option>Lengkap</option><option>Separa lengkap</option><option>Tidak lengkap</option><option>Tidak hadir</option></select></td><td><input class="score" type="number" min="0" max="10" step="1" value="10"></td><td class="suggest">TP6</td><td><input class="note" placeholder="Catatan"></td></tr>`).join('')||'<tr><td colspan="6">Tiada murid untuk kelas ini.</td></tr>';$$('#bookRows .score').forEach(inp=>inp.addEventListener('input',e=>e.target.closest('tr').querySelector('.suggest').textContent='TP'+suggestedTp(e.target.value)))}

async function saveTransit(){if(!requireAuth())return;
  const rows=$$('#transitRows tr[data-student]');const items=rows.map(r=>({student_id:r.dataset.student,class_id:$('#transitClass').value,subject_id:$('#transitSubject').value,standard_id:$('#transitStandard').value||null,assessment_date:$('#transitDate').value,tp:Number(r.querySelector('.tp').value)||null,evidence_note:r.querySelector('.note').value.trim(),source_type:'transit'})).filter(x=>x.tp);
  if(!items.length)return toast('Pilih sekurang-kurangnya satu TP.');
  if(state.connected&&state.user){items.forEach(x=>x.teacher_id=state.user.id);const {error}=await state.client.from('transit_records').upsert(items,{onConflict:'student_id,subject_id,standard_id,assessment_date'});if(error)return toast('Gagal simpan: '+error.message);await logAudit('SAVE_TRANSIT',{count:items.length});await loadAll()}
  else{const old=JSON.parse(localStorage.getItem('demo_transit')||'[]');items.forEach(x=>{x.id=crypto.randomUUID();x.created_at=new Date().toISOString()});localStorage.setItem('demo_transit',JSON.stringify([...items,...old]));state.transit=[...items,...old];renderDashboard();renderAnalytics()}
...
async function saveBooks(){if(!requireAuth())return;
  ...
  else{const old=JSON.parse(localStorage.getItem('demo_books')||'[]');items.forEach(x=>{x.id=crypto.randomUUID();x.created_at=new Date().toISOString()});localStorage.setItem('demo_books',JSON.stringify([...items,...old]));state.books=[...items,...old];renderDashboard()}
...
async function addSubject(){if(!requireAuth())return;const code=$('#newSubjectCode').value.trim().toUpperCase(),name=$('#newSubjectName').value.trim();if(!code||!name)return toast('Masukkan kod dan nama subjek.');if(state.connected&&state.user){const {error}=await state.client.from('subjects').insert({teacher_id:state.user.id,code,name});if(error)return toast('Gagal tambah subjek: '+error.message);await loadAll()}else{state.subjects.push({id:crypto.randomUUID(),code,name});hydrate()}$('#newSubjectCode').value='';$('#newSubjectName').value='';toast('Subjek berjaya ditambah.')}
async function addClass(){if(!requireAuth())return;const year=Number($('#newClassYear').value),name=$('#newClassName').value.trim();if(!name)return toast('Masukkan nama kelas.');if(state.connected&&state.user){const {error}=await state.client.from('classes').insert({teacher_id:state.user.id,name,year,academic_year:new Date().getFullYear()});if(error)return toast('Gagal tambah kelas: '+error.message);await loadAll()}else{state.classes.push({id:crypto.randomUUID(),name,year});hydrate()}$('#newClassName').value='';toast('Kelas berjaya ditambah.')}
async function addRphClassQuick(){
  ...
  else{created={id:crypto.randomUUID(),name,year,academic_year:ay};state.classes.push(created);hydrate()}
...
function smartSourceDocs(subjectId,year,academicYear=null,types=null){const allowed=types?new Set(types):null;return state.sources.map(d=>({...d,_smartScore:sourceDocMatchScore(d,subjectId,year,academicYear),_autoLinked:d.subject_id!==subjectId})).filter(d=>(!allowed||allowed.has(d.source_type))&&d._smartScore>=30).sort((a,b)=>b._smartScore-a._smartScore||String(b.created_at||'').localeCompare(String(a.created_at||'')))}
...
async function attemptTimetableImport(file,documentId,subjectFallback,classFallback){
  ...
  try{
    ...
    else state.timetable.push(...items.map(x=>({...x,id:crypto.randomUUID()})));
...
async function handleSourceFiles(type,files){if(!requireAuth())return;
  ...
  for(const file of files){
    let extraction={text:'',pageCount:null,method:'none',pages:[]},storagePath=null,storageStatus=state.connected?'pending':'demo',uploadError='',savedDocId=null;
    try{
      ...
      if(state.connected&&state.user){
        ...
        const id=crypto.randomUUID();savedDocId=id;state.sources.unshift({id,teacher_id:'demo',subject_id:subjectId,class_id:classId,year,academic_year:academicYear,source_type:type,title:file.name.replace(/\.[^.]+$/,''),file_name:file.name,mime_type:file.type||'',file_size:file.size,storage_path:null,extraction_status:chunks.length?'indexed_session':(extraction.method==='image-no-ocr'?'stored_no_ocr':'stored_only'),extracted_chars:Math.min(extraction.text.length,MAX_INDEX_CHARS),page_count:extraction.pageCount,created_at:new Date().toISOString()});state.sourceChunks.push(...chunks.map(x=>({...x,id:crypto.randomUUID(),document_id:id})));state.sourcePages.push(...(extraction.pages||[]).filter(p=>p.content).map(p=>({...p,id:crypto.randomUUID(),document_id:id,metadata:{kind:p.kind||'page'}})))
      }
...
async function loadStoredPdf(doc){if(!state.client||!state.user)throw new Error('Sambungan Supabase diperlukan.');if(!doc.storage_path)throw new Error('Fail asal tiada dalam Storage. Upload semula sumber ini.');const {data,error}=await state.client.storage.from(doc.storage_bucket||'source-files').createSignedUrl(doc.storage_path,900);if(error)throw error;const r=await fetch(data.signedUrl);if(!r.ok)throw new Error('Gagal mengambil PDF asal dari Storage.');const ab=await r.arrayBuffer();return await pdfjsLib.getDocument({data:ab}).promise}
...
async function deleteSource(id){const d=state.sources.find(x=>x.id===id);if(!d)return;if(!confirm(`Buang sumber ${d.file_name||d.title}?`))return;if(state.connected&&state.user){if(d.storage_path)await state.client.storage.from(d.storage_bucket||'source-files').remove([d.storage_path]);const {error}=await state.client.from('source_documents').delete().eq('id',id);if(error)return toast('Gagal buang: '+error.message);await loadAll()}else{state.sources=state.sources.filter(x=>x.id!==id);state.sourceChunks=state.sourceChunks.filter(x=>x.document_id!==id);state.sourcePages=state.sourcePages.filter(x=>x.document_id!==id);renderSources();renderDashboard();renderSourceReadiness();renderRphBadges()}toast('Sumber dibuang.')}
...
function renderRphBadges(){const sub=$('#rphSubject')?.value,cls=getClass($('#rphClass')?.value),year=cls?.year,ay=Number(cls?.academic_year||String($('#rphDate')?.value||today).slice(0,4)||new Date().getFullYear());renderRphClassHelper();if(!sub||!year){if($('#rphSourceBadges'))$('#rphSourceBadges').innerHTML=!state.classes.length?'<span class="source-badge">○ Tambah kelas untuk aktifkan RPH</span>':'';renderRphGate(null);return}const required=['rpt','dskp','textbook','timetable'],baCount=sourceCount('activity_book',sub,year,ay),week=Number($('#rphWeek')?.value||1),maps=state.lessonMaps.filter(x=>x.subject_id===sub&&Number(x.year)===Number(year)&&Number(x.academic_year)===ay&&Number(x.week_no)===week&&x.verification_status==='verified').length;$('#rphSourceBadges').innerHTML=required.map(k=>{const n=sourceCount(k,sub,year,ay);return `<span class="source-badge ${n?'have':''}">${n?'✓':'○'} ${SOURCE_TYPES[k]}${n?` ${n}`:''}</span>`}).join('')+`<span class="source-badge ${baCount?'have':''}">${baCount?'✓':'○'} Buku Aktiviti${baCount?` ${baCount}`:''} <small>(opsyenal)</small></span>`+`<span class="source-badge ${maps?'have':''}">${maps?'✓':'○'} Lesson Map Disahkan ${maps}</span>`;renderRphLessonOptions();renderRphGate(null)}

async function getChunksForSubject(subjectId,year,academicYear=null){
  const docs=smartSourceDocs(subjectId,year,academicYear);if(!docs.length)return[];const ids=docs.map(x=>x.id);
  if(state.connected&&state.user){
    const [{data:chunks,error:ce},{data:pages,error:pe}]=await Promise.all([
      state.client.from('source_chunks').select('document_id,chunk_no,content').in('document_id',ids).limit(3000),
      state.client.from('source_pages').select('document_id,page_no,content,metadata').in('document_id',ids).limit(6000)
    ]);
  ...
  const out=state.sourceChunks.filter(c=>ids.includes(c.document_id)).map(c=>({...c,doc:docs.find(d=>d.id===c.document_id)}));
  const pageRows=state.sourcePages.filter(p=>ids.includes(p.document_id)&&String(p.content||'').trim()).map(p=>({document_id:p.document_id,chunk_no:100000+Number(p.page_no||0),content:p.content,metadata:p.metadata||{},_page_no:p.page_no,doc:docs.find(d=>d.id===p.document_id)}));
  return out.concat(pageRows);
}

async function getPagesForSubject(subjectId,year,types=['textbook','activity_book'],academicYear=null){
  const docs=smartSourceDocs(subjectId,year,academicYear,types);if(!docs.length)return[];const ids=docs.map(x=>x.id);
  if(state.connected&&state.user){const {data,error}=await state.client.from('source_pages').select('document_id,page_no,content,metadata').in('document_id',ids).limit(6000);if(error){console.warn(error);return[]}return applyKnownBookProfilePages(annotatePrintedPages((data||[]).map(p=>({...p,doc:docs.find(d=>d.id===p.document_id)}))))}
  return applyKnownBookProfilePages(annotatePrintedPages(state.sourcePages.filter(p=>ids.includes(p.document_id)).map(p=>({...p,doc:docs.find(d=>d.id===p.document_id)}))))
}
...
function extractPageRefs(text){const clean=normalizeText(text);const out=[];const re=/(?:student(?:'s)?\s*book|pupil(?:'s)?\s*book|textbook|buku\s*teks|page|pages|pg|p\.|m\/?s|ms)\s*[:.]?\s*(\d{1,3})(?:\s*[-–—]\s*(\d{1,3}))?/ig;let m;while((m=re.exec(clean))){let a=Number(m[1]),b=Number(m[2]||m[1]);if(a>0&&a<500&&b>=a&&b-a<=8){for(let n=a;n<=b;n++)out.push(n)}}return [...new Set(out)].slice(0,12)}
...
function lessonLanguage(subjectId){const sub=getSubject(subjectId);const key=normKey(`${sub?.code||''} ${sub?.name||''}`);if(/\b(english|bahasa inggeris|bi|eng)\b/.test(key))return'en';return'ms'}
...
function exactBookTaskLines(text='',limit=8){
  ...
  const task=/^(?:bimbing\s+murid|minta\s+murid|murid\s+|baca(?:lah)?\b|jawab\b|tulis\b|padankan\b|lengkapkan\b|bina\b|kenal\s+pasti\b|lakukan\b|gunakan\b|salin\b|nyatakan\b|jelaskan\b|pilih\b|bulatkan\b|gariskan\b|warnakan\b|susun\b|catat\b|dengar\b|sebut\b|tutur\b|ceritakan\b|lakonkan\b|simulasikan\b|membina\b|menulis\b|membaca\b|memberikan\b|mengenal\b|mempersembahkan\b|melafazkan\b|menyanyikan\b|bercerita\b|bertutur\b|menjawab\b|melengkapkan\b|memadankan\b|menggunakan\b|menyusun\b|mencatat\b|menghasilkan\b|create\b|read\b|write\b|answer\b|match\b|complete\b|identify\b|listen\b|say\b|ask\b|tell\b|choose\b|tick\b|circle\b|draw\b|look\b|work\b|practise\b|practice\b|role[- ]?play\b|present\b)/i;
...
async function buildLessonCandidate(){if(!requireAuth())return;
  ...
  const bookHeading=detectBookHeading(bt?.content||'');if(!exactRptSession&&suspiciousTitle(title)&&bookHeading)title=bookHeading;if(!exactRptSession&&suspiciousTitle(title)&&unitInfo.topic)title=unitInfo.topic;if(exactRptSession?.title)title=cleanLessonTitle(exactRptSession.title);
...
function buildSourceActivities(map,ev,classId){
  const uiEn=lessonLanguage(map.subject_id)==='en',title=cleanLessonTitle(map.title||''),mapActs=String(map.source_activities||'').split('\n').map(cleanSourceActivityPhrase).filter(Boolean),siblings=siblingWeekActivityLines(map),chosen=[];
  const add=s=>{s=cleanSourceActivityPhrase(s);if(!s)return;const same=chosen.some(x=>jaccard(s,x)>=0.88);if(same)return;const siblingRepeat=siblings.some(x=>jaccard(s,x)>=0.92);if(siblingRepeat&&!/^RPT\s*(?:\+|sesi)/i.test(s))return;chosen.push(s)};
...
function buildSourceAwarePedagogy(map,activities,btRef,uiEn){const clean=activities.map(cleanSourceAnchor).filter(Boolean),anchor=clean[0]||cleanSourceAnchor(map.source_activities)||map.title||'',kind=sourceTaskKind(clean),page=btRef||'—',topic=map.title||'',method={oral:'Think-Pair-Share + Role Play',reading:'Pair Reading + Think-Pair-Share',writing:'Think-Write-Pair-Share',language:'Pair Check + Card Sort',arts:'Performance Circle',product:'Gallery Walk',general:'Think-Pair-Share'}[kind];let pakDetail,diffSupport,diffCore,diffChallenge;if(uiEn){const task=`“${anchor}”`;pakDetail=`${method}: use the exact textbook task ${task} (${page}). Pupils complete the source action first, then compare/check the same response with a partner; sharing stays on “${topic}” and the same Learning Standard.`;diffSupport=`Same textbook task ${task}: reduce source items/examples and provide cue words or one model from the same page.`;diffCore=`Complete ${task} exactly as required on ${page}, using the examples/text on the page as evidence.`;diffChallenge=`After ${task}, add one new response/example in the same “${topic}” context and explain the choice without changing the Learning Standard.`}else{const task=`“${anchor}”`;pakDetail=`${method}: gunakan tugasan sebenar buku teks ${task} (${page}). Murid melaksanakan tindakan sumber dahulu, kemudian membanding/menyemak tugasan yang sama dengan pasangan; perkongsian kekal pada tajuk “${topic}” dan SP yang sama.`;diffSupport=`Tugasan buku teks yang sama ${task}: kurangkan bilangan item/contoh sumber dan beri kata kunci atau satu contoh daripada halaman yang sama.`;diffCore=`Laksanakan ${task} seperti arahan pada ${page} dengan menggunakan contoh/teks pada halaman sebagai bukti.`;diffChallenge=`Selepas ${task}, bina satu respons/contoh tambahan dalam konteks “${topic}” dan jelaskan pilihan tanpa menukar SP.`}return {method,pakDetail,diffSupport,diffCore,diffChallenge,anchor,kind}}
...
async function generateRph(){if(!requireAuth())return;
  ...
  <h3>${uiEn?'Source-based Learning Activities':'Aktiviti PdP khusus sumber'}</h3><div class="activity">${escapeHtml(numbered)}</div>
  <h3>${uiEn?'21st Century Learning (source-aware)':'PAK-21 berpandukan sumber'}</h3><div class="group-card"><b>${escapeHtml(pedagogy.method)}</b><br><small>${escapeHtml(pedagogy.pakDetail)}</small></div>
  <h3>${uiEn?'Differentiated Learning (not based on PBD bands)':'PdP Terbeza berpandukan tugasan sumber'}</h3><div class="group-suggestion"><div class="group-card"><b>${uiEn?'Support':'Sokongan'}</b><br>${escapeHtml(pedagogy.diffSupport)}</div><div class="group-card"><b>${uiEn?'Core':'Aras Standard'}</b><br>${escapeHtml(pedagogy.diffCore)}</div><div class="group-card"><b>${uiEn?'Challenge':'Cabaran'}</b><br>${escapeHtml(pedagogy.diffChallenge)}</div></div>
  <h3>${uiEn?'Classroom Assessment (PBD)':'PBD'}</h3><p>${uiEn?'Assessment evidence comes from the same source task and Learning Standards. The teacher records the final mastery level in Transit PBD; differentiation is not generated from TP bands.':'Evidens PBD diambil daripada tugasan sumber dan Standard Pembelajaran yang sama. Guru merekod TP akhir dalam Transit PBD; PdP terbeza tidak dijana berdasarkan kumpulan TP.'}</p>
