...
async function loadAll(){if(!requireAuth())return;
  ...
  const qs=await Promise.all([
    ...
    fetchAllRows(c.from('rph_records').select('*').order('lesson_date',{ascending:false})),
    fetchAllRows(c.from('rph_activity_history').select('*').order('lesson_date',{ascending:false}))
  ]);
...
async function handleSourceFiles(type,files){if(!requireAuth())return;
  ...
  for(const file of files){
    let extraction={text:'',pageCount:null,method:'none',pages:[]},storagePath=null,storageStatus='pending',uploadError='',savedDocId=null;
    try{
...
async function getPagesForSubject(subjectId,year,types=['textbook','activity_book'],academicYear=null){
  ...
  if(state.connected&&state.user){const {data,error}=await fetchAllRows(state.client.from('source_pages').select('document_id,page_no,content,metadata').in('document_id',ids).order('page_no'));if(error){console.warn(error);return[]}return applyKnownBookProfilePages(annotatePrintedPages((data||[]).map(p=>({...p,doc:docs.find(d=>d.id===p.document_id)}))))}
  return []
}
...
function buildSourceAwarePedagogy(map,activities,btRef,uiEn){const clean=activities.map(cleanSourceAnchor).filter(Boolean),anchor=clean[0]||cleanSourceAnchor(map.source_activities)||map.title||'',kind=sourceTaskKind(clean),page=btRef||'—',topic=map.title||'',method={oral:'Think-Pair-Share + Role Play',reading:'Pair Reading + Think-Pair-Share',writing:'Think-Write-Pair-Share',language:'Pair Check + Card Sort',arts:'Performance Circle',product:'Gallery Walk',general:'Think-Pair-Share'}[kind];const bbmList=extractBBM(map,activities,btRef,uiEn);let pakDetail,diffSupport,diffCore,diffChallenge,setInduksi,penutup;if(uiEn){const task=`\u201c${anchor}\u201d`;pakDetail=`${method}: use the exact textbook task ${task} (${page}). Pupils complete the source action first, then compare/check the same response with a partner; sharing stays on \u201c${topic}\u201d and the same Learning Standard.`;diffSupport=`Explorer Group — Same textbook task ${task}: reduce source items/examples and provide cue words or one model from the same page.`;diffCore=`Builder Group — Complete ${task} exactly as required on ${page}, using the examples/text on the page as evidence.`;diffChallenge=`Challenger Group — After ${task}, add one new response/example in the same \u201c${topic}\u201d context and explain the choice without changing the Learning Standard.`;setInduksi=`Teacher displays a ${kind==='oral'?'short dialogue clip':'visual related to'} \u201c${topic}\u201d from ${page}. Pupils observe and share prior knowledge via Think-Pair-Share. Teacher probes with 2-3 guiding questions to activate schemata.`;penutup=`Teacher recaps the main SP ${map.source_evidence?.meta?.main_sp||''} using a quick-check: each group presents one key takeaway. Teacher distributes a self-reflection slip (\u201cI can...\u201d statement) and assigns follow-up practice if needed.`}else{const task=`\u201c${anchor}\u201d`;pakDetail=`${method}: gunakan tugasan sebenar buku teks ${task} (${page}). Murid melaksanakan tindakan sumber dahulu, kemudian membanding/menyemak tugasan yang sama dengan pasangan; perkongsian kekal pada tajuk \u201c${topic}\u201d dan SP yang sama.`;diffSupport=`Kelompok Peneroka — Tugasan buku teks yang sama ${task}: kurangkan bilangan item/contoh sumber dan beri kata kunci atau satu contoh daripada halaman yang sama.`;diffCore=`Kelompok Pembina — Laksanakan ${task} seperti arahan pada ${page} dengan menggunakan contoh/teks pada halaman sebagai bukti.`;diffChallenge=`Kelompok Pencabar — Selepas ${task}, bina satu respons/contoh tambahan dalam konteks \u201c${topic}\u201d dan jelaskan pilihan tanpa menukar SP.`;setInduksi=`Guru memaparkan ${kind==='oral'?'klip dialog pendek':'bahan visual berkaitan'} \u201c${topic}\u201d daripada ${page}. Murid memerhati dan berkongsi pengetahuan sedia ada melalui Think-Pair-Share. Guru bertanya 2-3 soalan bimbingan untuk mencetuskan pemikiran.`;penutup=`Guru merumuskan semula SP utama ${map.source_evidence?.meta?.main_sp||''} melalui kuiz pantas: setiap kumpulan membentangkan satu hasil pembelajaran utama. Guru mengagihkan borang refleksi kendiri (\u201cSaya boleh...\u201d) dan memberikan latihan susulan jika perlu.`}return {method,pakDetail,diffSupport,diffCore,diffChallenge,setInduksi,penutup,anchor,kind,bbmList}}
function extractBBM(map,activities,btRef,uiEn){const bbm=[];const page=btRef||'';bbm.push(uiEn?`Student's Book ${page}`:`Buku Teks ${page}`);if(map.activity_book_ref&&map.source_evidence?.meta?.activity_book_uploaded)bbm.push(uiEn?`Workbook ${map.activity_book_ref}`:`Buku Aktiviti ${map.activity_book_ref}`);if(map.source_evidence?.textbook)bbm.push(uiEn?'Source pages from uploaded documents':'Petikan halaman daripada dokumen yang diupload');const hay=normKey(activities.join(' '));if(/poster|peta minda|lukis/.test(hay))bbm.push(uiEn?'Poster / mind map materials':'Poster / bahan peta minda');if(/kad|card|matching/.test(hay))bbm.push(uiEn?'Cards / flashcards':'Kad imlak / kad flash');if(/lagu|nyanyi|audio/.test(hay))bbm.push(uiEn?'Audio / song clip':'Audio / klip lagu');if(/video|klip|tayang/.test(hay))bbm.push(uiEn?'Video clip':'Klip video');if(!bbm.some(b=>/kertas kerja|lembaran kerja|worksheet/i.test(b)))bbm.push(uiEn?'Worksheet / exercise paper':'Lembaran kerja');return bbm}
...
async function generateRph(){if(!requireAuth())return;
  ...
  <div class="rph-activity-block"><div class="rph-activity-label">${uiEn?'Source-based Activities':'Aktiviti Berasaskan Buku Teks'}</div><div class="rph-source-badge">📖 ${uiEn?'Based on':'Berdasarkan'} ${escapeHtml(btRef)}</div><div class="activity">${escapeHtml(numbered)}</div><div class="rph-source-links"><span class="rph-source-tag">${uiEn?'RPT':'RPT'}</span><span class="rph-source-tag">${uiEn?'DSKP':'DSKP'}</span><span class="rph-source-tag">${uiEn?'Student\'s Book':'Buku Teks'}</span></div></div>
  <div class="rph-activity-block"><div class="rph-activity-label">${uiEn?'21st Century Skills (PAK-21)':'Kemahiran Abad ke-21 (PAK-21)'}</div><div class="group-card"><b>${escapeHtml(pedagogy.method)}</b><br><small>${escapeHtml(pedagogy.pakDetail)}</small></div></div>
  <div class="rph-activity-block"><div class="rph-activity-label">${uiEn?'Differentiated Instruction':'PdP Terbeza'}</div><div class="group-suggestion"><div class="group-card group-explorer"><b>${uiEn?'Explorer Group':'Kelompok Peneroka'}</b><br><small>${uiEn?'(guided support — reduced scope, cue words, one model)':'(bimbingan — skop dikurangkan, kata kunci, satu contoh)'}</small><br>${escapeHtml(pedagogy.diffSupport)}</div><div class="group-card group-builder"><b>${uiEn?'Builder Group':'Kelompok Pembina'}</b><br><small>${uiEn?'(standard task — complete as per textbook page)':'(tugasan standard — lengkap seperti halaman buku teks)'}</small><br>${escapeHtml(pedagogy.diffCore)}</div><div class="group-card group-challenger"><b>${uiEn?'Challenger Group':'Kelompok Pencabar'}</b><br><small>${uiEn?'(extension — add new response, explain reasoning)':'(pengayaan — tambah respons baru, jelaskan sebab)'}</small><br>${escapeHtml(pedagogy.diffChallenge)}</div></div></div>
  <div class="rph-activity-block rph-bbm-inline"><div class="rph-activity-label">${uiEn?'Teaching Aids (BBM)':'Bahan Bantu Mengajar (BBM)'}</div><div class="rph-bbm-note">${uiEn?'BBM used to enhance PdP activities based on the textbook page:':'BBM digunakan untuk memperkayakan aktiviti PdP berdasarkan halaman buku teks:'}</div><div class="rph-bbm">${pedagogy.bbmList.map(b=>`<span class="rph-bbm-item">${escapeHtml(b)}</span>`).join('')}</div></div>
  </div></div>

  <div class="rph-section">
  <div class="rph-section-header"><span class="rph-section-num">3</span><h3>${uiEn?'Closure':'Penutup'}</h3></div><div class="rph-section-body"><div class="rph-closure">${escapeHtml(pedagogy.penutup)}</div></div></div>

  <div class="rph-section">
  <div class="rph-section-header"><span class="rph-section-num">4</span><h3>${uiEn?'Classroom Assessment (PBD)':'Pentaksiran Bilik Darjah (PBD)'}</h3></div><div class="rph-section-body"><p>${uiEn?'Assessment evidence comes from the same source task and Learning Standards. The teacher records the final mastery level in Transit PBD.':'Evidens PBD diambil daripada tugasan sumber dan Standard Pembelajaran yang sama. Guru merekod TP akhir dalam Transit PBD.'}</p></div></div>

  <div class="rph-section rph-bbm-section">
  <div class="rph-section-header"><span class="rph-section-num">5</span><h3>${uiEn?'Teaching Aids & Resources (BBM)':'Bahan Bantu Mengajar & Sumber (BBM)'}</h3></div><div class="rph-section-body"><p class="rph-bbm-note">${uiEn?'BBM are supplementary teaching aids to enhance PdP. Activities are based on RPT, DSKP and Student\'s Book. BBM can be used as notes, activities, or supporting materials.':'BBM adalah alat bantu mengajar untuk memperkayakan PdP. Aktiviti adalah berasaskan RPT, DSKP dan Buku Teks. BBM boleh digunakan sebagai nota, aktiviti, atau bahan sokongan.'}</p><div class="rph-bbm-grid"><div class="rph-bbm-card"><div class="rph-bbm-card-title">${uiEn?'As Notes':'Sebagai Nota'}</div><div class="rph-bbm-card-items">${pedagogy.bbmList.slice(0,2).map(b=>`<span class="rph-bbm-item">${escapeHtml(b)}</span>`).join('')||'<span class="rph-bbm-item rph-bbm-empty">—</span>'}</div></div><div class="rph-bbm-card"><div class="rph-bbm-card-title">${uiEn?'As Activities':'Sebagai Aktiviti'}</div><div class="rph-bbm-card-items">${pedagogy.bbmList.slice(2,4).map(b=>`<span class="rph-bbm-item">${escapeHtml(b)}</span>`).join('')||'<span class="rph-bbm-item rph-bbm-empty">—</span>'}</div></div><div class="rph-bbm-card"><div class="rph-bbm-card-title">${uiEn?'As Supporting Materials':'Sebagai Bahan Sokongan'}</div><div class="rph-bbm-card-items">${pedagogy.bbmList.slice(4).map(b=>`<span class="rph-bbm-item">${escapeHtml(b)}</span>`).join('')||'<span class="rph-bbm-item rph-bbm-empty">—</span>'}</div></div></div><div class="rph-bbm-source-link">${uiEn?'All BBM linked to':'Semua BBM dikaitkan dengan'} ${escapeHtml(btRef)} (${uiEn?'Student\'s Book':'Buku Teks'})</div></div></div>
...
$('#transitClass').addEventListener('change',()=>{refreshStandards();renderTransitLessonOptions()});$('#transitSubject').addEventListener('change',()=>{refreshStandards();renderTransitLessonOptions()});$('#transitLesson')?.addEventListener('change',()=>{const map=state.lessonMaps.find(x=>x.id===$('#transitLesson').value);renderTransitMeta(map);renderTransitRows()});$('#refreshTransitStandards').addEventListener('click',()=>{refreshStandards();renderTransitLessonOptions()});
