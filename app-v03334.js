...
function validateRphMap(map,ev,acts){const hasBtActivities=Number(acts.exactTextbookCount||0)>=1;const hasRptActivities=!hasBtActivities&&acts.activities.length>=1&&acts.activities.some(x=>/^RPT\b/i.test(x));const checks=[{n:'Lesson Map disahkan',ok:map.verification_status==='verified'},{n:'Source Match ≥ 85%',ok:Number(map.confidence_score)>=85},{n:'Buku Teks berhalaman',ok:!!map.textbook_page_start&&(ev.bt.length>0||hasRptActivities)},{n:'Aktiviti sumber dikesan',ok:hasBtActivities||hasRptActivities},{n:'Minggu & SK/SP cross-check',ok:Boolean(map.week_exact)&&Boolean(map.sp_crosscheck)},{n:'SK/SP tersedia',ok:!!map.sk&&!!map.sp},{n:'SP Utama ditetapkan',ok:!!(map.source_evidence?.meta?.main_sp)},{n:'Objektif & kriteria lengkap',ok:!!map.objective&&!!map.success_criteria},{n:'Aktiviti khusus sumber',ok:acts.activities.length>=1},{n:'Anti-repeat terkawal',ok:acts.similarity<0.90||hasBtActivities||hasRptActivities}];if(ev.hasBA)checks.push({n:'Buku Aktiviti opsyenal',ok:true});const score=Math.round(checks.filter(x=>x.ok).length/checks.length*100);return {checks,score}}
...
async function generateRph(){if(!requireAuth())return;
  ...
  <div class="rph-activity-block"><div class="rph-activity-label">${uiEn?'Differentiated Instruction (3 Groups)':'PdP Terbeza (3 Kumpulan)'}</div><div class="group-suggestion"><div class="group-card group-explorer"><b>${uiEn?'Explorer Group':'Kelompok Peneroka'}</b><br><small>${uiEn?'(guided support — collaboration & communication)':'(bimbingan — kerjasama & komunikasi)'}</small><br><div class="rph-diff-activity"><b>${uiEn?'Activity:':'Aktiviti:'}</b> ${escapeHtml(pedagogy.diffSupportAct)}</div><div class="rph-diff-desc"><b>${uiEn?'Description:':'Penerangan:'}</b> ${escapeHtml(pedagogy.diffSupport)}</div></div><div class="group-card group-builder"><b>${uiEn?'Builder Group':'Kelompok Pembina'}</b><br><small>${uiEn?'(standard task — critical thinking & problem-solving)':'(tugasan standard — berpikir kritis & menyelesaikan masalah)'}</small><br><div class="rph-diff-activity"><b>${uiEn?'Activity:':'Aktiviti:'}</b> ${escapeHtml(pedagogy.diffCoreAct)}</div><div class="rph-diff-desc"><b>${uiEn?'Description:':'Penerangan:'}</b> ${escapeHtml(pedagogy.diffCore)}</div></div><div class="group-card group-challenger"><b>${uiEn?'Challenger Group':'Kelompok Pencabar'}</b><br><small>${uiEn?'(extension — creativity & innovation)':'(pengayaan — kreativiti & inovasi)'}</small><br><div class="rph-diff-activity"><b>${uiEn?'Activity:':'Aktiviti:'}</b> ${escapeHtml(pedagogy.diffChallengeAct)}</div><div class="rph-diff-desc"><b>${uiEn?'Description:':'Penerangan:'}</b> ${escapeHtml(pedagogy.diffChallenge)}</div></div></div></div>
