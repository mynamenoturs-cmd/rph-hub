...
function sourceTasks(ped){
  ...
  for(const step of ped?.sourceSteps||[]){
    const t=cleanTask(step?.rawText||step?.text||'');if(t&&t.length>=6&&!out.includes(t))out.push(t);
  }
...
function materialize(v,tasks=[]){
  const summary=taskSummary(tasks),first=tasks[0]||'';
  return norm(String(v||'').replace(/\{\{\s*source_activity\s*\}\}/gi,summary||first));
}
...
function libraryMainStep(row,map,ped,tasks=[]){
  ...
  const parsed=parseDifferentiation(materialize(row?.example_text,tasks)),fallback=fallbackDiff(page,tasks);
  const diff={support:parsed.support||fallback.support,core:parsed.core||fallback.core,challenge:parsed.challenge||fallback.challenge};
  const base=materialize(row?.activity_template,tasks);
  const text=`${base} PdP Terbeza dalam aktiviti yang sama: Peneroka — ${diff.support} Pembina — ${diff.core} Pencabar — ${diff.challenge}`;
  const bbm=materialize(row?.bbm_template,tasks)||page;
  return {key:String(row?.activity_key||'science-y3-library'),name:norm(row?.activity_name||'Aktiviti Sains'),phase,duration:durationFor(phase),text:norm(text),bbm,pak21:norm(row?.pak21||'Kolaborasi • Komunikasi'),libraryActivityKey:row?.activity_key||'',libraryDriven:true};
}
...
if(typeof previousBuild==='function'){
  const enhanced=function(map,activities,btRef,uiEn,classId=null){
    ...
    try{
      ...
      ped.activityLibrarySelection={activity_key:row.activity_key,activity_name:row.activity_name,phase:row.phase,source:'rph_activity_library',version:VERSION};
...
window.__RPH_SCIENCE_Y3_LIBRARY_FLOW__={version:VERSION,subject:'science',year:3,expectedRows:EXPECTED_LIBRARY_ROWS,libraryDeterminesMethod:true,sourceDeterminesContent:true,forcedGameEverySession:false,separateDifferentiationBlock:false};
