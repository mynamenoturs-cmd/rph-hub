(function(){
  ...
  function pair(map,task){
    ...
    if(hasWrite&&hasListen)return{objective:`By the end of the lesson, pupils can write a relevant description based on the activity in ${p} and listen to a friend's description attentively.`,criteria:`Pupils write at least one relevant sentence based on their picture or source task and respond to at least one detail from a friend's description.`};
    if(hasDraw&&hasWrite)return{objective:`By the end of the lesson, pupils can complete the picture task in ${p} and write a relevant sentence or short description about it.`,criteria:`Pupils complete the picture task and write at least one relevant sentence that matches the picture and the instruction in ${p}.`};
    if(hasListen)return{objective:`By the end of the lesson, pupils can listen to the text or description in ${p} and identify at least one relevant detail correctly.`,criteria:`Pupils identify or respond to at least one detail that can be checked against the listening task in ${p}.`};
    if(hasRead)return{objective:`By the end of the lesson, pupils can read the material in ${p} and identify at least one relevant piece of information correctly.`,criteria:`Pupils give at least one correct answer and point to the word, phrase or sentence in ${p} that supports it.`};
    if(hasWrite)return{objective:`By the end of the lesson, pupils can complete the writing task in ${p} with at least one relevant and understandable sentence.`,criteria:`Pupils write at least one sentence that follows the instruction in ${p} and is understandable in context.`};
    if(hasSpeak)return{objective:`By the end of the lesson, pupils can give an appropriate spoken response based on the task in ${p}.`,criteria:`Pupils give at least one relevant spoken response using information or language from ${p}.`};
    return{objective:`By the end of the lesson, pupils can complete the task “${t.replace(/[.!?]+$/,'')}” in ${p} accurately.`,criteria:`Pupils complete the task in ${p} and produce an answer or response that can be checked directly against the source.`};
  }
  ...
  window.__RPH_ENGLISH_QUALITY_HOTFIX__={version:'2026-09-06e'};
